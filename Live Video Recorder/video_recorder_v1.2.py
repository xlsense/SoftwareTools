...
parser_desc = "Records files from RTSP stream using ffmpeg v5.1 or newer. For older ffmpeg builds, replace -timeout with -stimeout in the ffmpeg command in the script."
parser = argparse.ArgumentParser(description=parser_desc, formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
# comment out the below command for FFmpeg builds version 4.x and earlier
command_line = "{0} -timeout 5000000 -i {1} -an -vcodec copy -segment_time {2} -f segment -strftime 1 -hide_banner -loglevel quiet -stats {3}".format(args.ffmpeg_path, stream_url, args.segtime, output_format)

# uncomment the below command one above for FFmpeg builds version 4.x and earlier
